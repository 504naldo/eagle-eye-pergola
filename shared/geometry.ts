...
function withRate(item: Omit<QTOItem, "unitRate" | "lineTotal">, rateOverride?: number): QTOItem {
  const unitRate = rateOverride ?? DEFAULT_RATES[item.description] ?? 0;
...
export function calculateQTO(p: PergolaParams): QTOItem[] {
  ...
  const items: QTOItem[] = [
    ...
    withRate({
      ...
      description:
        p.slatType === "fixed"
          ? "Fixed aluminum slats (150×25)"
          : "Operable aluminum louvers (150×25)",
      unit: "EA",
      qty: slatCount,
    ...
    ...(totalGlassM2 > 0
      ? [
          ...
          withRate({
            ...
            unit: "LM",
            qty: Math.round(glassPerimeterM * 10) / 10,
